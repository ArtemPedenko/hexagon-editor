...
2. **Toolbar и toolbar items — done; editor presets остаются.** Локальный фиксированный toolbar заменён публичной системой item/group/config factories с конфигурируемым порядком, action bindings и availability; применимые `zero`, `commonmark`, `default`, `full` toolbar presets подключены. Остаётся перенос одноимённых editor presets/specs отдельным срезом. Исключённые Autocomplete, CommandMenu и YFM extensions в presets не входят.
