...
| `src/modules/toolbars/*`, `src/toolbar/*`, `src/plugins/*` | `src/toolbar/*`, Vue toolbar/components | partial | Публичные item/group config, default/minimal presets, порядок и контекстная availability подключены к runtime; остаются upstream item factories, plugin-driven actions и toolbar presets commonmark/full. |
...
2. **Toolbar, toolbar items и presets — partial.** Локальный фиксированный toolbar заменён публичной системой items/groups с конфигурируемым порядком и availability; default/minimal presets подключены. Остаются upstream item factories, plugin-driven actions и перенос `commonmark`, `full` и применимой части `yfm` presets/specs. Исключённые Autocomplete, CommandMenu и YFM extensions в эти presets не входят.
