...
test.describe('Markdown editor playground', () => {
  test('renders every advanced Markdown extension without console errors', async ({ page }) => {
    ...
    await expect(page.locator('.ProseMirror p', { hasText: '<div data-demo-html>Raw HTML block</div>' })).toHaveText(
    ...
    await expect(page.locator('.ProseMirror [data-yfm-html]')).toHaveText(
  ...
  test('inserts and resizes an image from the playground toolbar', async ({ page }) => {
    ...
    await expect(
      page.locator(
        '[data-markdown-editor-toolbar] [data-toolbar-item="image-width"], [data-markdown-editor-toolbar] [data-toolbar-item="image-fit"]',
      ),
    ).toHaveCount(0);
  ...
  test('prefills link text from the visual editor selection', async ({ page }) => {
    ...
    const paragraph = page
      ...
      .filter({ hasText: 'Этот раздел можно свернуть кнопкой в тулбаре.' });
  ...
  test('sets a language and shows line numbers for a code block', async ({ page }) => {
    ...
    await expect(
      page.locator('.ProseMirror pre[data-language="javascript"] .markdown-editor__code-line-number'),
    ).toHaveText(['1', '2']);
  ...
  test('inserts HTML after the current visual block and opens its local editor', async ({ page }) => {
    ...
    const insertedAfterParagraph = await paragraph.evaluate((element) =>
      element.nextElementSibling?.classList.contains('markdown-editor__atomic-source'),
    );
  ...
  test('groups inline and block code actions in one toolbar popover', async ({ page }) => {
    ...
    await expect(
      page
        .locator('.ProseMirror pre[data-markup]')
        .filter({ hasText: 'Этот раздел можно свернуть кнопкой в тулбаре.' }),
    ).toBeVisible();
  ...
  test('keeps table actions readable in the dark theme', async ({ page }) => {
    ...
    await expect(page.getByRole('menu', { name: 'Действия с таблицей' })).toHaveCSS(
      'background-color',
      'rgb(30, 32, 36)',
    );
    await expect(page.getByRole('menuitem', { name: 'Добавить строку' })).toHaveCSS('color', 'rgb(241, 243, 245)');
    await expect(page.getByRole('menuitem', { name: 'Добавить строку' })).toHaveCSS(
      'background-color',
      'rgba(0, 0, 0, 0)',
    );
  });
  ...
  test('inserts an editable 3 by 3 table from the toolbar', async ({ page }) => {
    ...
    await expect(page.locator('.ProseMirror table').last().locator('td')).toHaveCount(8);
    await expect(page.locator('.ProseMirror table').last().locator('td').first()).toHaveCSS(
      'border-top-style',
      'solid',
    );
  });
  ...
  test('folds a section from its heading gutter', async ({ page }) => {
    ...
    await expect(
      page.locator('.ProseMirror p').filter({ hasText: 'Этот раздел можно свернуть кнопкой в тулбаре.' }),
  ...
  test('preserves folding heading attributes when changing editor modes', async ({ page }) => {
    ...
    await selectEditorMode(page, 'Разметка');
    await expect(page.locator('.markdown-editor[data-mode="markup"] .cm-content')).toContainText(
...
test.describe('Documentation', () => {
  test('renders every registered route with its heading, active link, and typed example', async ({ page }) => {
    for (const [slug, title] of documentationPages) {
      ...
      await expect(
        page.locator('.docs-article pre code').filter({ hasText: '<script setup lang="ts">' }).first(),
      ).toBeVisible();
    }
  });
