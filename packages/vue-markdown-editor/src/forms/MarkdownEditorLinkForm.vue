...
export interface MarkdownEditorLinkSubmitParams {
...
<template>
	...
	>
		...
		<MarkdownEditorTextInput
			:model-value="text"
			:aria-label="copy.linkText"
			:disabled="disabled"
			:help="copy.linkTextHelp"
			:label="copy.linkText"
			:readonly="readOnlyText"
			@update:model-value="emit('update:text', $event)"
		/>
		<label class="markdown-editor__form-checkbox">
			<input
				:checked="openInNewWindow"
				:disabled="disabled"
				type="checkbox"
				@change="emit('update:openInNewWindow', ($event.target as HTMLInputElement).checked)"
			/>
			{{ copy.linkNewWindow }}
		</label>
		<template #footer>
...
<style scoped>
.markdown-editor__form-checkbox {
	display: flex;
	align-items: center;
	gap: 0.5rem;
	cursor: pointer;
}
.markdown-editor__form-checkbox input {
	width: 1rem;
	height: 1rem;
	margin: 0;
	accent-color: #5282ff;
