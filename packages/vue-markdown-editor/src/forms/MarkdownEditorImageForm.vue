...
export interface MarkdownEditorImageSubmitParams {
	alt: string;
	height?: number;
	name: string;
	title: string;
...
const emit = defineEmits<{
	apply: [params: MarkdownEditorImageSubmitParams];
	cancel: [];
...
function submit(): void {
	...
	if (!isValidUrl(url)) {
		urlError.value = copy.value.imageUrlError;
		emit('invalid', urlError.value);
		return;
	}
	urlError.value = '';
	const params: MarkdownEditorImageSubmitParams = {
		alt: props.alt.trim(),
		name: props.name.trim(),
		title: props.title.trim(),
		url,
	};
	const width = optionalDimension(props.width);
	const height = optionalDimension(props.height);
...
function isValidUrl(value: string): boolean {
	...
	try {
		return value.length > 0 && new URL(value).protocol.match(/^https?:$/) !== null;
	} catch {
		return false;
	}
}
function optionalDimension(value: number | string): number | undefined {
	const parsed = Number(value);
	return value === '' || !Number.isFinite(parsed) || parsed < 0 ? undefined : parsed;
}
...
<template>
	...
	>
		<label
			v-if="uploadImage"
			class="markdown-editor-image-upload"
			:class="{
				'markdown-editor-image-upload--disabled': disabled || uploading,
			}"
		>
			<input
				accept="image/*"
		...
		<p
			v-if="uploadError"
			class="markdown-editor-image-upload__error"
			role="alert"
		>
			{{ uploadError }}
		</p>
		...
		<MarkdownEditorTextInput
			:model-value="name"
			:disabled="disabled"
			:label="copy.imageName"
			@update:model-value="emit('update:name', $event)"
		/>
		<MarkdownEditorTextInput
			:model-value="alt"
			:aria-label="copy.imageAltAria"
			:disabled="disabled"
			:help="copy.imageAltHelp"
			:label="copy.imageAlt"
		...
		/>
		<div
			class="markdown-editor-form__sizes"
			:aria-label="copy.imageSizes"
		>
			<MarkdownEditorTextInput
				:model-value="width"
				:aria-label="copy.imageWidth"
				:disabled="disabled"
				:min="0"
				:placeholder="copy.imageWidth"
				type="number"
			...
			<MarkdownEditorTextInput
				:model-value="height"
				:aria-label="copy.imageHeight"
				:disabled="disabled"
				:min="0"
				:placeholder="copy.imageHeight"
				type="number"
...
.markdown-editor-image-upload {
	...
	min-height: 2.25rem;
	padding: 0 0.75rem;
	border: 1px dashed var(--markdown-border);
	border-radius: 0.375rem;
	cursor: pointer;
}
.markdown-editor-image-upload:hover,
.markdown-editor-image-upload:focus-within {
	border-color: var(--markdown-focus-text);
}
.markdown-editor-image-upload--disabled {
	cursor: default;
	opacity: 0.55;
}
.markdown-editor-image-upload input {
	position: absolute;
	width: 1px;
	height: 1px;
	overflow: hidden;
...
.markdown-editor-image-upload__error {
	margin: -0.35rem 0 0;
	color: #d7504b;
	font-size: 0.75rem;
}
