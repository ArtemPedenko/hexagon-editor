...
export function createToolbarGroup(id: string, items: MarkdownEditorToolbarGroup['items']): MarkdownEditorToolbarGroup {
	return { id, items };
}

export function createToolbarConfig(groups: MarkdownEditorToolbarConfig['groups']): MarkdownEditorToolbarConfig {
	return { groups };
}

export const zeroToolbarConfig = createToolbarConfig([createToolbarGroup('history', ['undo', 'redo'])]);
...
export function getToolbarConfig(preset: MarkdownEditorToolbarPreset): MarkdownEditorToolbarConfig {
	switch (preset) {
		case 'zero':
			return zeroToolbarConfig;
		case 'commonmark':
			return commonmarkToolbarConfig;
		case 'full':
