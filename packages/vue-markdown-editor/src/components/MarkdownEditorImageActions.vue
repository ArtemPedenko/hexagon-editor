...
<template>
  <div
    ref="element"
    class="markdown-editor__image-actions"
    :data-theme="theme"
    role="toolbar"
  ...
  >
    <button
      type="button"
      :aria-label="translate('imageWidthFull')"
      :title="translate('imageWidthFull')"
      @mousedown.prevent
      @click="emit('full-width')"
    >
      ↔
    </button>
    <select
      :value="objectFit ?? 'contain'"
      :aria-label="translate('imageFit')"
      @change="emit('object-fit', ($event.target as HTMLSelectElement).value as ImageObjectFit)"
    >
...
.markdown-editor__image-actions[data-theme='dark'] {
...
@media (prefers-color-scheme: dark) {
  .markdown-editor__image-actions[data-theme='auto'] {
    --markdown-background: #303236;
    --markdown-border: #45484e;
    --markdown-focus-background: #526da8;
    --markdown-focus-text: #fff;
