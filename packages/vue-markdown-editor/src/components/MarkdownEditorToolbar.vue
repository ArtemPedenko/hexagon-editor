...
<template>
  <div class="markdown-editor__toolbar" data-markdown-editor-toolbar role="toolbar" aria-label="Форматирование Markdown">
   <div v-for="group in (toolbarConfig ?? getToolbarConfig(toolbarPreset)).groups" :key="group.id" class="markdown-editor__toolbar-group" role="group" :data-toolbar-group="group.id">
    ...
    <button v-if="toolbarItem.id === 'undo'" data-toolbar-item="undo" type="button" :aria-label="translate('undo')" :title="translate('undo')" @mousedown.prevent @click="emit('execute', commands.undo)">↶</button>
    <button v-else-if="toolbarItem.id === 'redo'" data-toolbar-item="redo" type="button" :aria-label="translate('redo')" :title="translate('redo')" @mousedown.prevent @click="emit('execute', commands.redo)">↷</button>
    ...
    <button v-else-if="toolbarItem.id === 'bold'" data-toolbar-item="bold" :aria-label="translate('bold')" :aria-pressed="state.bold" type="button" :title="translate('bold')" @mousedown.prevent @click="emit('execute', commands.bold)"><strong>B</strong></button>
    <button v-else-if="toolbarItem.id === 'italic'" data-toolbar-item="italic" :aria-label="translate('italic')" :aria-pressed="state.italic" type="button" :title="translate('italic')" @mousedown.prevent @click="emit('execute', commands.italic)"><em>I</em></button>
    <button v-else-if="toolbarItem.id === 'underline'" data-toolbar-item="underline" :aria-pressed="state.underline" type="button" title="Подчёркивание" @mousedown.prevent @click="emit('execute', commands.underline)"><u>U</u></button>
    <button v-else-if="toolbarItem.id === 'strike'" data-toolbar-item="strike" :aria-pressed="state.strikethrough" type="button" title="Зачёркивание" @mousedown.prevent @click="emit('execute', commands.strikethrough)"><s>S</s></button>
    <button v-else-if="toolbarItem.id === 'mark'" data-toolbar-item="mark" :aria-pressed="state.mark" type="button" title="Выделить" @mousedown.prevent @click="emit('execute', commands.mark)">▣</button>
    <button v-else-if="toolbarItem.id === 'code'" data-toolbar-item="code" :aria-label="translate('code')" :aria-pressed="state.code" type="button" :title="translate('code')" @mousedown.prevent @click="emit('execute', commands.code)">&lt;/&gt;</button>
    <button v-else-if="toolbarItem.id === 'bullet-list'" data-toolbar-item="bullet-list" :aria-pressed="state.bulletList" type="button" title="Маркированный список" @mousedown.prevent @click="emit('execute', commands.bulletList)">•≡</button>
    <button v-else-if="toolbarItem.id === 'ordered-list'" data-toolbar-item="ordered-list" :aria-pressed="state.orderedList" type="button" title="Нумерованный список" @mousedown.prevent @click="emit('execute', commands.orderedList)">1≡</button>
    <button v-else-if="toolbarItem.id === 'quote'" data-toolbar-item="quote" :aria-pressed="state.quote" type="button" title="Цитата" @mousedown.prevent @click="emit('execute', commands.quote)">❝</button>
