  ...
  <div class="markdown-editor__toolbar" data-markdown-editor-toolbar role="toolbar" aria-label="Форматирование Markdown">
    <div v-for="group in (toolbarConfig ?? getToolbarConfig(toolbarPreset)).groups" :key="group.id" class="markdown-editor__toolbar-group" role="group" :data-toolbar-group="group.id">
      <template v-for="toolbarItem in availableItems(group.items)" :key="toolbarItem.id">
        ...
        <button v-else-if="toolbarItem.id === 'underline'" data-toolbar-item="underline" :disabled="!isToolbarItemEnabled(toolbarItem, state)" :aria-pressed="isToolbarItemActive(toolbarItem, state, state.underline)" type="button" title="Подчёркивание" @mousedown.prevent @click="runItem(toolbarItem, commands.underline)"><ToolbarIcon name="underline" /></button>
        <button v-else-if="toolbarItem.id === 'strike'" data-toolbar-item="strike" :disabled="!isToolbarItemEnabled(toolbarItem, state)" :aria-pressed="isToolbarItemActive(toolbarItem, state, state.strikethrough)" type="button" title="Зачёркивание" @mousedown.prevent @click="runItem(toolbarItem, commands.strikethrough)"><ToolbarIcon name="strike" /></button>
        <button v-else-if="toolbarItem.id === 'mark'" data-toolbar-item="mark" :disabled="!isToolbarItemEnabled(toolbarItem, state)" :aria-pressed="isToolbarItemActive(toolbarItem, state, state.mark)" type="button" title="Выделить" @mousedown.prevent @click="runItem(toolbarItem, commands.mark)">M</button>
        ...
        <button v-else-if="toolbarItem.id === 'bullet-list'" data-toolbar-item="bullet-list" :disabled="!isToolbarItemEnabled(toolbarItem, state)" :aria-pressed="isToolbarItemActive(toolbarItem, state, state.bulletList)" type="button" title="Маркированный список" @mousedown.prevent @click="runItem(toolbarItem, commands.bulletList)"><ToolbarIcon name="bulletList" /></button>
        ...
        <button v-else-if="toolbarItem.id === 'quote'" data-toolbar-item="quote" :disabled="!isToolbarItemEnabled(toolbarItem, state)" :aria-pressed="isToolbarItemActive(toolbarItem, state, state.quote)" type="button" title="Цитата" @mousedown.prevent @click="runItem(toolbarItem, commands.quote)"><ToolbarIcon name="quote" /></button>
        ...
        <button v-else-if="toolbarItem.id === 'image'" data-toolbar-item="image" :aria-expanded="imageEditorVisible" type="button" title="Изображение" @mousedown.prevent @click="emit('toggle-image-editor', getButton($event))"><ToolbarIcon name="image" /></button>
        ...
        <button v-else-if="toolbarItem.id === 'horizontal-rule'" data-toolbar-item="horizontal-rule" type="button" title="Горизонтальная линия" @mousedown.prevent @click="emit('execute', commands.horizontalRule)"><ToolbarIcon name="horizontalRule" /></button>
        <button v-else-if="toolbarItem.id === 'table'" data-toolbar-item="table" type="button" title="Таблица 3×3" @mousedown.prevent @click="emit('execute', commands.insertTable())"><ToolbarIcon name="table" /></button>
      </template>
    </div>
