...
describe('MarkdownEditor', () => {
  ...
  it('keeps v-model, both split hosts, and mode changes in sync', async () => {
    ...
    markupView?.dispatch({
      changes: {
        from: 0,
        to: markupView.state.doc.length,
        insert: '# Updated',
      },
    });
    await nextTick();
  ...
  it('localizes labels, applies the selected theme, and switches modes from the toolbar', async () => {
    ...
    expect(target.querySelector('[data-markdown-editor-toolbar]')?.getAttribute('aria-label')).toBe(
      'Markdown formatting',
    );
    modeButton.click();
    await nextTick();
  ...
  it('changes a heading back to ordinary text from the heading picker', async () => {
    ...
    Array.from(document.body.querySelectorAll<HTMLButtonElement>('[role="menuitemradio"]'))
      .find((button) => button.textContent?.includes('Текст'))
      ?.click();
    await nextTick();
  ...
  it('applies toolbar commands to the markup editor', async () => {
    ...
    markupView?.focus();
    markupView?.dispatch({
      selection: { anchor: 0, head: modelValue.value.length },
    });
  ...
  it('highlights only the nearest list type in the list popover', async () => {
    ...
    const nestedText = Array.from(target.querySelectorAll('li'))
      .find((item) => item.textContent === 'Nested')
    ...
    expect(
      Array.from(document.body.querySelectorAll<HTMLButtonElement>('[role="menuitem"]')).find((item) =>
        item.textContent?.includes('Увеличить отступ'),
      )?.disabled,
    ).toBe(true);
    expect(
      Array.from(document.body.querySelectorAll<HTMLButtonElement>('[role="menuitem"]')).find((item) =>
        item.textContent?.includes('Уменьшить отступ'),
      )?.disabled,
    ).toBe(false);
  ...
  it('renders custom toolbar groups in the configured order and availability', async () => {
      ...
      h(MarkdownEditor, {
        toolbarConfig: {
          groups: [
            ...
            {
              id: 'custom-second',
              items: [{ id: 'table', isAvailable: () => false }, 'link'],
            },
          ],
        },
      }),
    ...
    expect(
      Array.from(target.querySelectorAll('[data-toolbar-group]')).map((group) =>
        group.getAttribute('data-toolbar-group'),
      ),
    ).toEqual(['custom-first', 'custom-second']);
    expect(
      Array.from(target.querySelectorAll('[data-toolbar-item]')).map((button) =>
        button.getAttribute('data-toolbar-item'),
      ),
    ).toEqual(['italic', 'bold', 'link', 'mode']);
          ...
          groups: [
            {
              ...
              items: [
                ...
                {
                  id: 'italic',
                  action: { isEnabled: () => false, run: vi.fn() },
                },
              ],
            },
          ],
  ...
  it('shows raw HTML and unspaced HTML directives as source text', async () => {
    ...
    const app = createApp(() =>
      h(MarkdownEditor, {
        modelValue:
          '<section data-raw-html-element>Raw HTML</section>\n\n:::html\n<section data-yfm-html-element>YFM HTML</section>\n:::',
      }),
    );
    ...
    expect(target.querySelector('.ProseMirror p')?.textContent).toBe(
      '<section data-raw-html-element>Raw HTML</section>',
    );
    expect(target.querySelector('[data-yfm-html]')?.textContent).toBe(
      ':::html\n<section data-yfm-html-element>YFM HTML</section>\n:::',
    );
  ...
  it('inserts an HTML directive after the current block and opens its local editor', async () => {
    ...
    expect(target.querySelector('.markdown-editor__atomic-source')?.textContent).toContain(
      '<div>Add HTML code here</div>',
    );
