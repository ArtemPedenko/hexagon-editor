...
function configureMath(markdown: MarkdownIt): void {
    markdown.inline.ruler.after('backticks', 'inline_math', (state, silent) => {
        if (state.src[state.pos] !== '$' || state.src[state.pos + 1] === '$' || isEscaped(state.src, state.pos)) return false;
...
function configureInlineTokens(children: Token[]): void {
    for (let index = 0; index < children.length; index += 1) {
        ...
        if (token?.type === 'image' && suffix?.type === 'text') {
            const dimensions = suffix.content.match(/^\{width=(\d+%?)(?:\s+height=(\d+))?(?:\s+object-fit=(contain|cover|fill|none|scale-down))?}/);
            if (dimensions !== null) {
...
function configureQuoteLinks(markdown: MarkdownIt): void {
    markdown.core.ruler.after('renderer_extensions', 'quote_links', (state) => {
        for (let index = 0; index < state.tokens.length; index += 1) {
            ...
            if (open?.type !== 'blockquote_open' || paragraph?.type !== 'paragraph_open' || inline?.type !== 'inline' || paragraphClose?.type !== 'paragraph_close') continue;
...
function configureRenderer(markdown: MarkdownIt): void {
    ...
    markdown.renderer.rules.html_block = (tokens, index) => `<div data-raw-html>${htmlEscape(tokens[index]?.content ?? '')}</div>\n`;
    ...
    markdown.renderer.rules.image = (tokens, index, options, environment, renderer) => {
        ...
        const styles: string[] = [];
        if (width !== null && width !== undefined && /^\d+%?$/.test(width)) styles.push(`width: ${width.endsWith('%') ? width : `${width}px`}`);
        ...
        return defaultImage?.(tokens, index, options, environment, renderer) ?? renderer.renderToken(tokens, index, options);
    };
    ...
    const defaultHeadingClose = markdown.renderer.rules.heading_close ?? ((tokens, index, options, _environment, renderer) => renderer.renderToken(tokens, index, options));
    ...
    markdown.renderer.rules.heading_close = (tokens, index, options, environment: RendererEnvironment, renderer) => {
        ...
        return defaultHeadingClose(tokens, index, options, environment, renderer) + (open?.attrGet('data-folding') === 'true' ? '</summary>' : '');
    };
}
