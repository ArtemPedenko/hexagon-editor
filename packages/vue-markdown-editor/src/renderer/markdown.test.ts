...
describe('renderMarkdownContent', () => {
	...
	it('renders formulas and keeps invalid formulas recoverable', () => {
		...
		const rendered = renderMarkdownContent('$x$', {
			math: {
				renderToString: (latex, display) => `<span data-fake-math="${display}">${latex}</span>`,
			},
		});
	...
	it('groups folding headings into nested interactive sections', () => {
		...
		expect(result).toContain(
			'<details data-folding-section open><summary><h2 data-folding="true">Parent</h2>\n</summary>',
		);
		expect(result).toContain(
			'<details data-folding-section open><summary><h3 data-folding="true">Child</h3>\n</summary>',
		);
