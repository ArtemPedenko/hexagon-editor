...
export const boldMarkSpec: MarkSpec = {
    ...
    parseDOM: [
        {tag: 'b'},
        {getAttrs: (node) => ({[BoldAttrs.Markup]: node.getAttribute(BoldAttrs.Markup)}), tag: 'strong'},
        {getAttrs: (value) => /^(bold(er)?|[5-9]\d{2,})$/.test(value as string) && null, style: 'font-weight'},
    ],
