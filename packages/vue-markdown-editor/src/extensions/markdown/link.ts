...
export const removeLink: Command = (state, dispatch, view) => {
	...
	return link.isInSet(state.selection.$from.marks()) === undefined ? false : toggleMark(link)(state, dispatch, view);
};

export const toggleLink =
	(href: string, title?: string): Command =>
	(state, dispatch, view) =>
		toggleMark(getLinkType(state.schema), {
			[LinkAttr.Href]: href,
...
export const setLink =
	...
	(state, dispatch) => {
		...
		if (
			(range === undefined && state.selection.empty) ||
			(text !== undefined && text !== state.doc.textBetween(from, to))
		) {
			transaction = transaction.insertText(content, from, to);
		}
		const end = from + content.length;
		transaction = transaction.removeMark(from, end, link).addMark(
			from,
			end,
			link.create({
				[LinkAttr.Href]: href,
...
export function getCurrentLink(state: EditorState):
	| {
			href: string;
...
export const Link: ExtensionAuto<LinkOptions> = (builder, options) => {
	...
	if (options?.linkKey !== null && options?.linkKey !== undefined) {
		builder.addKeymap(({ schema }) => ({
			[options.linkKey!]: toggleMark(getLinkType(schema)),
		}));
	}
	builder
		.addInputRules(({ schema }) => ({
			rules: [linkInputRule(getLinkType(schema))],
		}))
		.addPlugin(
			() =>
				new Plugin({
...
function getLinkRange(
	state: EditorState,
	link: MarkType,
): { from: number; mark: ReturnType<MarkType['create']>; to: number } | undefined {
	const { $from } = state.selection;
	const mark =
		link.isInSet($from.marks()) ??
		link.isInSet($from.nodeBefore?.marks ?? []) ??
		link.isInSet($from.nodeAfter?.marks ?? []);
	if (mark === undefined) return undefined;
	const parentStart = $from.start();
	const children: Array<{
		nodeSize: number;
		offset: number;
		marks: readonly ReturnType<MarkType['create']>[];
	}> = [];
	$from.parent.forEach((node, offset) => children.push({ marks: node.marks, nodeSize: node.nodeSize, offset }));
	const hasSameLink = (marks: readonly ReturnType<MarkType['create']>[]): boolean =>
		marks.some((candidate) => candidate.eq(mark));
	const cursor = state.selection.from;
	const index = children.findIndex(
		({ marks, nodeSize, offset }) =>
			parentStart + offset <= cursor && cursor <= parentStart + offset + nodeSize && hasSameLink(marks),
	);
...
function linkInputRule(markType: MarkType): InputRule {
	return new InputRule(/\[(.+)]\((\S+)\)\s$/, (state, match, start, end) => {
		...
		if (alt === undefined || href === undefined || (code !== undefined && state.doc.rangeHasMark(start, end, code)))
			return null;
		return state.tr
			.replaceWith(start, end, markType.schema.text(alt))
			.addMark(start, start + alt.length, markType.create({ [LinkAttr.Href]: href }));
	});
}
