...
export const toggleBold: Command = (state, dispatch, view) =>
	toggleMark(getBoldType(state.schema))(state, dispatch, view);
...
export const Bold: ExtensionAuto<BoldOptions> = (builder, options) => {
	...
	if (options?.boldKey !== null && options?.boldKey !== undefined) {
		builder.addKeymap(({ schema }) => ({
			[options.boldKey!]: toggleMark(getBoldType(schema)),
		}));
	}
...
export function createMarkdownMarkInputRule(
	rule: { close: string; ignoreBetween?: string; open: string },
	markType: MarkType,
): InputRule {
	...
	return new InputRule(expression, (state, match, start, end) => {
		...
		if (
			code !== undefined &&
			(state.doc.rangeHasMark(start, end, code) || state.selection.$from.marks().some((mark) => mark.type === code))
		)
			return null;
...
function getMarksBetween(
	start: number,
	end: number,
	state: EditorState,
): Array<{ end: number; mark: Mark; start: number }> {
