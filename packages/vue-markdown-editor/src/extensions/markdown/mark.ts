...
export const toggleHighlight: Command = (state, dispatch, view) => toggleMark(getMarkType(state.schema))(state, dispatch, view);
...
export const Mark: ExtensionAuto = (builder) => {
    builder
        ...
        .addInputRules(({schema}) => ({rules: [createMarkdownMarkInputRule({close: '==', ignoreBetween: '=', open: '=='}, getMarkType(schema))]}));
};
