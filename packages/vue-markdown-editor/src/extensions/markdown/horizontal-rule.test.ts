...
describe('HorizontalRule extension', () => {
  it('preserves horizontal rule Markdown markup', () => {
    const result = ExtensionsManager.process((builder) => builder.use(HorizontalRule), {
  ...
  it('inserts a paragraph after a horizontal rule from non-empty text', () => {
    ...
    expect(
      addHorizontalRule(basicMarkdownSchema.nodes.horizontal_rule!)(state, (transaction) => {
