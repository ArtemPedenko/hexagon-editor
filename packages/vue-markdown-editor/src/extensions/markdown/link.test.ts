...
describe('Link extension', () => {
	it('parses and serializes a link with title', () => {
		const result = ExtensionsManager.process((builder) => builder.use(Link), {
	...
	it('escapes parentheses in a link URL', () => {
		...
		const documentNode = basicMarkdownSchema.node('doc', null, [
			basicMarkdownSchema.node(
				'paragraph',
				null,
				basicMarkdownSchema.text('site', [
					basicMarkdownSchema.marks.link.create({
						[LinkAttr.Href]: 'https://example.com/a(b)',
					}),
				]),
			),
		]);
	...
	it('adds a link mark to selected text', () => {
		...
		expect(
			toggleLink('https://example.com')(state, (transaction) => {
	...
	it('updates and removes a selected link', () => {
		const documentNode = basicMarkdownSchema.node('doc', null, [
			basicMarkdownSchema.node(
				...
				basicMarkdownSchema.text('site', [
					basicMarkdownSchema.marks.link.create({
						[LinkAttr.Href]: 'https://old.example.com',
					}),
				]),
			),
		]);
		...
		let removed = updated;
		expect(
			removeCurrentLink(updated, (transaction) => {
	...
	it('preserves surrounding text when an editing selection extends past the link', () => {
		const link = basicMarkdownSchema.marks.link.create({
			[LinkAttr.Href]: 'http://localhost:5173/',
			[LinkAttr.Title]: '3213123',
		});
		const documentNode = basicMarkdownSchema.node('doc', null, [
	...
	it('reads a link when the cursor is inside its text', () => {
		const documentNode = basicMarkdownSchema.node('doc', null, [
			basicMarkdownSchema.node(
				'paragraph',
				null,
				basicMarkdownSchema.text('localhost', [
					basicMarkdownSchema.marks.link.create({
						[LinkAttr.Href]: 'http://localhost:5173/',
					}),
				]),
		...
		const state = EditorState.create({
			doc: documentNode,
			selection: TextSelection.create(documentNode, 5),
		});
