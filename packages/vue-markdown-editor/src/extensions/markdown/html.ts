...
export const htmlNodeSpecs: Record<HtmlNode, NodeSpec> = {
	[HtmlNode.Block]: {
		...
		parseDOM: [
			{
				getAttrs: (node) => ({
					[HtmlAttr.Content]: (node as HTMLElement).getAttribute('data-html-raw') ?? (node as HTMLElement).innerHTML,
				}),
				tag: '[data-html]',
			},
		],
	...
	[HtmlNode.Inline]: {
		...
		parseDOM: [
			{
				getAttrs: (node) => ({
					[HtmlAttr.Content]: (node as HTMLElement).textContent,
				}),
				tag: 'span[data-html]',
			},
		],
...
export const htmlTokenSpecs: Record<HtmlNode, ParseSpec> = {
	[HtmlNode.Block]: {
		getAttrs: (token) => ({ [HtmlAttr.Content]: token.content }),
		noCloseToken: true,
		node: HtmlNode.Block,
	},
	[HtmlNode.Inline]: {
