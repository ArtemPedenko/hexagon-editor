...
describe('Image extension', () => {
    ...
    it('turns a pasted image URL into a full-width image', () => {
        ...
        const handled = plugin?.props.handlePaste?.({
            dispatch: (transaction) => { nextState = state.apply(transaction); },
            state,
        } as never, {
            clipboardData: {getData: () => 'https://example.com/photo.webp?size=large'},
            preventDefault,
        } as ClipboardEvent, false);

        expect(handled).toBe(true);
        expect(nextState.doc.firstChild?.firstChild?.attrs[ImageAttr.Src]).toBe('https://example.com/photo.webp?size=large');
