...
describe('Code extension', () => {
    it('parses and serializes inline code with backticks', () => {
        const result = ExtensionsManager.process(
            (builder) => builder.use(Code, {codeKey: 'Mod-`'}),
    ...
    it('uses additional backticks for code containing a backtick', () => {
        ...
        expect(ExtensionsManager.process((builder) => builder.use(Code), {baseSchema: basicMarkdownSchema}).serializer.serialize(parsed)).toBe('`` ` ``\n');
    });
    ...
    it('toggles the code mark for a text selection', () => {
        ...
        expect(toggleCode(state, (transaction) => {
