...
export const listTokenSpecs: Record<'bullet_list' | 'list_item' | 'ordered_list', ParseSpec> = {
  bullet_list: {
    ...
    getAttrs: (token, tokens, index) => ({
      markup: token.markup,
      tight: isListTight(tokens, index),
    }),
  },
  list_item: {
    block: 'list_item',
...
export const listSerializerNodes: Partial<
  Record<'bullet_list' | 'list_item' | 'ordered_list', ConstructorParameters<typeof MarkdownSerializer>[0][string]>
> = {
