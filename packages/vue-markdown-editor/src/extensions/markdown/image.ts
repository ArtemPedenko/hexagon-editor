...
export const imageNodeSpec: NodeSpec = {
  ...
  toDOM: (node) => {
    ...
    for (const [name, value] of Object.entries(node.attrs)) {
      if (value === null || name === ImageAttr.ObjectFit || name === ImageAttr.Width || name === ImageAttr.Height)
        continue;
    ...
    if (node.attrs[ImageAttr.Width] !== null)
      image.style.width = String(node.attrs[ImageAttr.Width]).includes('%')
        ? String(node.attrs[ImageAttr.Width])
        : `${node.attrs[ImageAttr.Width]}px`;
  ...
  markdown.core.ruler.after('inline', 'image_dimensions', (state) => {
    for (const token of state.tokens) {
      ...
      for (const [index, image] of token.children.entries()) {
        ...
        const match = suffix.content.match(
...
export const setImageDisplay =
  (attrs: { height?: number | null; objectFit?: ImageObjectFit; width?: number | string }): Command =>
  (state, dispatch) => {
