...
export const toggleCode: Command = (state, dispatch, view) => toggleMark(getCodeType(state.schema))(state, dispatch, view);
...
export const Code: ExtensionAuto<CodeOptions> = (builder, options) => {
    ...
    if (options?.codeKey !== null && options?.codeKey !== undefined) {
        builder.addKeymap(({schema}) => ({[options.codeKey!]: toggleMark(getCodeType(schema))}));
    }
    builder
