...
export {BreakNodeName, BreaksSpecs, breakTokenSpecs, hardBreakNodeSpec, serializeHardBreak, serializeSoftBreak, softBreakNodeSpec} from './breaks-specs';
...
export const addBreak =
    ...
    chainCommands(exitCode, (state, dispatch) => {
        ...
        if (isBreakNode(selection.$cursor.nodeBefore)) {
            ...
            const content = state.doc.slice(isBreakNode(selection.$cursor.nodeAfter) ? position + 1 : position, end, false).content;
...
export const Breaks: ExtensionAuto<BreaksOptions> = (builder, options) => {
    ...
    builder.addKeymap(() => ({'Shift-Enter': addBreak(preferredBreak === 'soft' ? BreakNodeName.SoftBreak : BreakNodeName.HardBreak)}));
};
