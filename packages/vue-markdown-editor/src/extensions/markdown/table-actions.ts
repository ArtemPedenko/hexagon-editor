...
export function createTable(schema: EditorState['schema'], rows = 3, columns = 3): ProseMirrorNode {
    ...
    const body = bodyType.create(null, Array.from({length: rows - 1}, () => createRow(dataCellType, columns)));
    return tableType.create(null, [head, body]);
}

export const insertTable = (rows = 3, columns = 3): Command => (state, dispatch) => {
    if (dispatch !== undefined) dispatch(state.tr.replaceSelectionWith(createTable(state.schema, rows, columns)).scrollIntoView());
    return true;
};
...
export const addTableRow: Command = (state, dispatch) => {
    ...
    dispatch?.(state.tr.insert(position, createDataRow(state.schema, selection.body.firstChild?.childCount ?? 1)).scrollIntoView());
    return true;
};
...
export const deleteTableRow: Command = (state, dispatch) => {
    ...
    if (selection.body.childCount === 1) {
        dispatch?.(state.tr.delete(selection.tablePosition, selection.tablePosition + selection.table.nodeSize).scrollIntoView());
    } else {
...
export const setTableColumnAlignment = (alignment: TableCellAlign): Command => (state, dispatch) => {
    const selection = getTableSelection(state);
    if (selection === undefined) return false;
    const transaction = state.tr;
    for (const row of tableRows(selection).reverse()) {
...
export const goToTableCell = (direction: 'next' | 'prev'): Command => (state, dispatch) => {
    const selection = getTableSelection(state);
    if (selection === undefined) return false;
