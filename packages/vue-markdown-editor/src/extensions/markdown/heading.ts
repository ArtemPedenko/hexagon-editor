...
export {HeadingSpecs, headingLevelAttr, headingLineNumberAttr, headingNodeName, headingNodeSpec, headingTokenSpec, serializeHeading} from './heading-specs';
...
export const resetHeading: Command = (state, dispatch, view) => {
    const paragraph = state.schema.nodes.paragraph;
    if (!state.selection.empty || paragraph === undefined || state.selection.$from.parent.type !== getHeadingType(state.schema)) {
        return false;
    }
...
export const Heading: ExtensionAuto<HeadingOptions> = (builder, options) => {
    ...
    builder
        ...
        .addInputRules(({schema}) => ({
            rules: [textblockTypeInputRule(/^(#{1,6})\s$/, getHeadingType(schema), (match) => ({[headingLevelAttr]: match[1]?.length}))],
        }));
};
