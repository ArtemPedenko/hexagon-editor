...
export const setCodeBlock: Command = (state, dispatch, view) =>
	setBlockType(getCodeBlockType(state.schema))(state, dispatch, view);

export const setCodeBlockLanguage =
	(language: string): Command =>
	(state, dispatch) => {
		const selectedNode =
			state.selection instanceof NodeSelection && state.selection.node.type === getCodeBlockType(state.schema)
				...
				: state.selection.$from.parent.type === getCodeBlockType(state.schema)
					? {
							attrs: state.selection.$from.parent.attrs,
							position: state.selection.$from.before(),
						}
					: undefined;
		if (selectedNode === undefined) return false;
		dispatch?.(
			state.tr.setNodeMarkup(selectedNode.position, undefined, {
				...selectedNode.attrs,
...
export const CodeBlock: ExtensionAuto<CodeBlockOptions> = (builder, options) => {
	...
	if (options?.codeBlockKey !== null && options?.codeBlockKey !== undefined) {
		builder.addKeymap(({ schema }) => ({
			[options.codeBlockKey!]: setBlockType(getCodeBlockType(schema)),
		}));
	}
};
			...
			decorations: (state) => {
				...
				state.doc.descendants((node, position) => {
					...
					for (const token of getSyntaxTokens(source, language)) {
						...
						decorations.push(
							Decoration.inline(position + token.from + 1, position + token.to + 1, {
								class: `markdown-editor__code-token--${token.kind}`,
							}),
						);
					}
					return false;
...
function createLineNumberDecoration(position: number, line: number): Decoration {
	return Decoration.widget(position, () => createLineNumber(line), {
		side: -1,
	});
}
...
function getSyntaxTokens(
	...
	from: number;
	kind: 'comment' | 'keyword' | 'number' | 'string' | 'tag';
	to: number;
}> {
...
function getSyntaxExpression(language: string): RegExp | undefined {
	if (['js', 'javascript', 'ts', 'typescript'].includes(language))
		return /\/\/.*$|\/\*[\s\S]*?\*\/|(['"`])(?:\\.|(?!\1)[^\\])*\1|\b(?:async|await|class|const|export|function|if|import|let|new|return|throw|type|interface|var)\b|\b\d+(?:\.\d+)?\b/gm;
	if (language === 'json') return /"(?:\\.|[^"\\])*"|\b(?:true|false|null)\b|\b\d+(?:\.\d+)?\b/g;
	if (['html', 'xml', 'vue'].includes(language))
		return /<!--[\s\S]*?-->|<\/?[\w-]+(?:\s[^<>]*)?>|(['"])(?:\\.|(?!\1)[^\\])*\1/g;
	if (language === 'css')
		return /\/\*[\s\S]*?\*\/|(['"])(?:\\.|(?!\1)[^\\])*\1|\b[\w-]+(?=\s*:)|#[\da-fA-F]{3,8}|\b\d+(?:\.\d+)?(?:px|rem|em|%)?/g;
	return undefined;
}
