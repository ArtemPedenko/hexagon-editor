...
describe('Blockquote extension', () => {
    it('registers schema, Markdown codec, input rule and Backspace keymap', () => {
        const result = ExtensionsManager.process(
            (builder) => builder.use(Blockquote, {qouteKey: 'Mod-Shift-b'}),
    ...
    it('wraps and unwraps a text cursor with the upstream toggle command', () => {
        ...
        const initial = EditorState.create({
            ...
            selection: TextSelection.create(basicMarkdownSchema.node('doc', null, [paragraph.create(null, basicMarkdownSchema.text('text'))]), 1),
        });
        let wrapped = initial;
        expect(toggleQuote(initial, (transaction) => {
        ...
        const quoted = EditorState.create({doc: wrapped.doc, schema: basicMarkdownSchema, selection});
        let unwrapped = quoted;
        expect(toggleQuote(quoted, (transaction) => {
    ...
    it('lifts a cursor from the first position in a blockquote', () => {
        ...
        const state = EditorState.create({doc: quoted, schema: basicMarkdownSchema, selection: TextSelection.create(quoted, 2)});
        let next = state;

        expect(liftFromQuote(state, (transaction) => {
