...
export {ItalicAttrs, ItalicSpecs, italicMarkName, italicMarkSpec, italicTokenSpec, serializeItalic} from './italic-specs';
...
export const toggleItalic: Command = (state, dispatch, view) => toggleMark(getItalicType(state.schema))(state, dispatch, view);
...
export const Italic: ExtensionAuto<ItalicOptions> = (builder, options) => {
    ...
    if (options?.italicKey !== null && options?.italicKey !== undefined) {
        builder.addKeymap(({schema}) => ({[options.italicKey!]: toggleMark(getItalicType(schema))}));
    }
};
