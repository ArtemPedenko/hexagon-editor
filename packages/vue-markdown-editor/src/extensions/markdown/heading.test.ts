...
describe('Heading extension', () => {
  it('parses and serializes heading levels', () => {
    const result = ExtensionsManager.process((builder) => builder.use(Heading, { h2Key: 'Mod-Alt-2' }), {
  ...
  it('toggles an equal heading level back to paragraph', () => {
    ...
    let heading = initial;
    expect(
      toHeading(2)(initial, (transaction) => {
    ...
    expect(
      toHeading(2)(selectedHeading, (transaction) => {
