...
function cellSpec(tag: 'th' | 'td'): NodeSpec {
  return {
    ...
    toDOM: (node) => [
      ...
      {
        ...node.attrs,
        style: `text-align:${node.attrs[TableAttrs.CellAlign]}`,
      },
      0,
    ],
  };
...
export const tableTokenSpecs: Record<TableNode, ParseSpec> = {
  ...
  [TableNode.Row]: {
    block: TableNode.Row,
    getAttrs: (token) => ({
      [TableAttrs.Line]: token.attrGet(TableAttrs.Line),
    }),
  },
  [TableNode.HeaderCell]: {
    block: TableNode.HeaderCell,
...
function getCellAttrs(token: { attrs: Array<[string, string]> | null }): Partial<Record<TableAttrs, string>> {
  ...
  const align = style
    .split(';')
    .find((rule) => rule.startsWith('text-align:'))
    ?.split(':')[1];
...
function renderCell(
  state: Parameters<ConstructorParameters<typeof MarkdownSerializer>[0][string]>[0],
  node: Parameters<ConstructorParameters<typeof MarkdownSerializer>[0][string]>[1],
): void {
  state.write('|');
