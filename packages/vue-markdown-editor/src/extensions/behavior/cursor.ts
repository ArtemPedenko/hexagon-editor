...
export class GapCursorSelection<Meta = unknown> extends Selection {
    ...
    get $pos(): ResolvedPos { return this.$head; }
    get pos(): number { return this.$head.pos; }
...
export function createGapCursorPlugin(): Plugin<boolean> {
    return new Plugin<boolean>({
        ...
        props: {
            ...
            decorations: ({doc, selection}) => isGapCursorSelection(selection)
                ? DecorationSet.create(doc, [Decoration.widget(selection.head, renderGapCursor, {key: 'gapcursor', side: -1})])
                : null,
        },
    });
}
...
export interface CursorOptions { dropOptions?: Parameters<typeof dropCursor>[0]; }
