				...
				props: {
					handleDOMEvents: {
						paste(view, event) {
							...
							const rawText =
								files.length > 0 ? files.map((file) => file.name).join(' ') : event.clipboardData.getData('text/plain');
							const text = view.state.selection.$from.parent.type.spec.code ? rawText : rawText.replaceAll('\n', '↵');
