...
describe('ClicksOnEdges', () => {
    ...
    it('does not handle a click above the document content', () => {
        const documentNode = basicMarkdownSchema.node('doc', null, [basicMarkdownSchema.node('math_block', {latex: 'x'})]);
        const plugin = ExtensionsManager.process((builder) => builder.use(ClicksOnEdges), {baseSchema: basicMarkdownSchema}).plugins[0];
        const view = new EditorView(document.createElement('div'), {state: EditorState.create({doc: documentNode, plugins: [plugin]})});

        const handled = plugin.props.handleClick?.(view, 0, {offsetY: -1, target: view.dom} as MouseEvent);
