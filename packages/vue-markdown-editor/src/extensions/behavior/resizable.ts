  ...
  builder.addPlugin(
    () =>
      new Plugin({
        props: {
          decorations: (state) => {
            if (!(state.selection instanceof NodeSelection) || state.selection.node.type.name !== imageNodeName)
...
function createResizeHandle(view: EditorView): HTMLElement {
  ...
  handle.addEventListener('pointerdown', (event) => {
    if (!(view.state.selection instanceof NodeSelection) || view.state.selection.node.type.name !== imageNodeName)
      return;
    ...
    const finish = (upEvent: PointerEvent) => {
      ...
      const attrs = {
        ...
        [ImageAttr.Height]: Math.round(width / ratio),
