...
function isEdgeTextblock($cursor: ResolvedPos, direction: SelectionDirection): boolean {
  ...
  return direction === 'before' ? index === 0 : index === $cursor.node($cursor.depth - 1).childCount - 1;
}
...
export function findFakeParaPosForCodeBlock($cursor: ResolvedPos, direction: SelectionDirection): ResolvedPos | null {
...
const arrow =
  (arrowDirection: ArrowDirection): Command =>
  (state, dispatch, view) => {
    ...
    if (isGapCursorSelection<GapCursorMeta>(selection)) {
      ...
      if (
        (arrowDirection === 'up' && selection.pos === 0) ||
        (arrowDirection === 'down' && selection.pos === state.doc.nodeSize - 2)
      )
        return true;
