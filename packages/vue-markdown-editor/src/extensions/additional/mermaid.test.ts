...
describe('Mermaid extension', () => {
    it('parses and serializes Mermaid fences', () => {
        const {serializer, textParser} = ExtensionsManager.process((builder) => builder.use(Mermaid), {baseSchema: basicMarkdownSchema});
    ...
    it('registers an action that inserts an editable Mermaid block', () => {
        ...
        const action = ExtensionsManager.process((builder) => builder.use(Mermaid), {baseSchema: basicMarkdownSchema}).actions.action(mermaidActionName);
