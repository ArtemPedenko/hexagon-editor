...
describe('YfmHtmlBlock extension', () => {
    it('parses and serializes an HTML directive', () => {
        const {serializer, textParser} = ExtensionsManager.process((builder) => builder.use(YfmHtmlBlock), {baseSchema: basicMarkdownSchema});
    ...
    it('registers an action that inserts an editable HTML block', () => {
        ...
        let state = EditorState.create({doc: document, selection: TextSelection.create(document, 1)});
        const action = ExtensionsManager.process((builder) => builder.use(YfmHtmlBlock), {baseSchema: basicMarkdownSchema}).actions.action(yfmHtmlBlockActionName);
