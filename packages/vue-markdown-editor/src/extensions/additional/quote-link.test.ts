...
describe('QuoteLink extension', () => {
    it('parses and serializes a QuoteLink block', () => {
        const {serializer, textParser} = ExtensionsManager.process((builder) => builder.use(QuoteLink), {baseSchema: basicMarkdownSchema});
        ...
        expect(document.firstChild?.attrs).toMatchObject({cite: 'https://example.com', content: 'Source'});
    ...
    it('registers an action that wraps and unwraps the current block', () => {
        const document = basicMarkdownSchema.node('doc', null, [basicMarkdownSchema.node('paragraph', null, basicMarkdownSchema.text('Quote'))]);
        let state = EditorState.create({doc: document, selection: TextSelection.create(document, 2)});
        const action = ExtensionsManager.process((builder) => builder.use(QuoteLink), {baseSchema: basicMarkdownSchema}).actions.action(quoteLinkActionName);

        expect(action?.isEnabled({state})).toBe(true);
        action?.run({state, dispatch: (transaction) => { state = state.apply(transaction); }});
