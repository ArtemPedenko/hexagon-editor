...
export const toggleFoldingHeading: Command = (state, dispatch) => {
	...
	if (dispatch !== undefined) {
		...
		const transaction =
			folding === null
				? insertTrailingParagraph(state, position, headingLevel($from.parent)).setNodeMarkup(position, undefined, {
						...$from.parent.attrs,
						folding: false,
					})
...
export const removeFoldingAtHeadingStart: Command = (state, dispatch) => {
	...
	if (
		...
		cursor.parent.attrs.folding === null
	)
		return false;
	dispatch?.(
		state.tr
			.setNodeMarkup(cursor.before(), undefined, {
				...cursor.parent.attrs,
				folding: null,
			})
			.scrollIntoView(),
	...
	builder
		.addAction('toggleHeadingFolding', () => ({
			isActive: (context?: unknown) =>
				isFoldingHeadingActionContext(context) && context.state.selection.$from.parent.attrs.folding !== null,
		...
		.addInputRules(() => ({
			rules: [new InputRule(/^(#{1,6})\+\s$/, foldingHeadingRule)],
		}))
		.addKeymap(
			() => ({
				Backspace: removeFoldingAtHeadingStart,
				Enter: openHeadingAndCreateParagraphAfter,
			}),
					...
					props: {
						...
						handleClickOn: (view, _position, node, nodePosition, event) => {
							...
							if (
								...
								event.clientX - heading.getBoundingClientRect().left >= FOLDING_GUTTER_WIDTH
							)
								return false;
							const state = view.state.apply(
								view.state.tr.setSelection(TextSelection.create(view.state.doc, nodePosition + 1)),
							);
