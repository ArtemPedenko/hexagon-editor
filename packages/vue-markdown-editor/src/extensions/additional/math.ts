...
export const mathNodeSpecs: Record<MathNode, NodeSpec> = {
	[MathNode.Inline]: {
		...
		group: 'inline',
		inline: true,
		toDOM: (node) => ['span', { 'data-math-inline': '' }, `$${node.attrs.latex}$`],
	},
	[MathNode.Block]: {
		atom: true,
		attrs: { latex: { default: '' } },
		group: 'block',
		toDOM: (node) => ['pre', { 'data-math-block': '' }, `$$\n${node.attrs.latex}\n$$`],
	},
};

export function createMathNodeSpecs(
	render: (latex: string, display: boolean) => HTMLElement,
): Record<MathNode, NodeSpec> {
	return {
		[MathNode.Inline]: {
			...mathNodeSpecs[MathNode.Inline],
			toDOM: (node) => render(node.attrs.latex, false),
		},
		[MathNode.Block]: {
			...mathNodeSpecs[MathNode.Block],
			toDOM: (node) => render(node.attrs.latex, true),
		},
	};
}
		...
		props: {
			handleDOMEvents: {
				paste: (view, event) => {
					...
					if (data === null || data === undefined || view.state.selection.$from.parent.type.spec.code === true)
						return false;
					...
					view.dispatch(
						view.state.tr
							.replaceWith(view.state.selection.from, view.state.selection.to, Fragment.from(nodes))
							.scrollIntoView(),
					);
...
export const insertMathBlock: Command = (state, dispatch) => {
	...
	if (type === undefined) return false;
	dispatch?.(
		state.tr.replaceWith($from.before(), $from.after(), type.create({ latex: defaultMathLatex })).scrollIntoView(),
	);
	return true;
};
...
export const moveCursorLeftOfMathInline: Command = (state, dispatch) => {
	...
	if (
		...
		$from.parentOffset === 0 ||
		nodeBefore === null ||
		nodeBefore.type !== state.schema.nodes[MathNode.Inline]
	)
		return false;
...
export const moveCursorRightOfMathInline: Command = (state, dispatch) => {
	...
	if (!$from.parent.isTextblock || nodeAfter === null || nodeAfter.type !== state.schema.nodes[MathNode.Inline])
		return false;
...
export const selectMathInlineBeforeCursor: Command = (state, dispatch) => {
	const { $from } = state.selection;
	const nodeBefore = $from.nodeBefore;
	if (!$from.parent.isTextblock || nodeBefore === null || nodeBefore.type !== state.schema.nodes[MathNode.Inline])
		return false;
...
function isInCode(state: EditorState, from: number, to: number): boolean {
	...
	return (
		code !== undefined &&
		(state.doc.rangeHasMark(from, to, code) || state.selection.$from.marks().some((mark) => mark.type === code))
	);
}
...
export const Math: ExtensionAuto = (builder) => {
	...
	for (const node of Object.values(MathNode)) {
		builder
			.addNodeSpec(node, () => mathNodeSpecs[node])
			.addMarkdownTokenParserSpec(node, () => mathTokenSpecs[node])
			.addNodeSerializerSpec(node, () => mathSerializerNodes[node]);
	}
	builder
