...
export function getBasicWysiwygSelectionState(
  ...
  const codeBlock =
    selectedNode?.type.name === 'code_block'
      ? selectedNode
  ...
  return {
    bold: hasActiveMark(state, schema, 'strong'),
    bulletList: listType === 'bullet_list',
    code: hasActiveMark(state, schema, 'code'),
    codeBlock: codeBlock !== undefined,
    codeBlockLanguage: codeBlock?.attrs[CodeBlockAttrs.Lang] as string | undefined,
    ...
    imageObjectFit: image?.attrs['object-fit'] as string | undefined,
    italic: hasActiveMark(state, schema, 'em'),
    linkHref: currentLink?.href,
    linkOpenInNewWindow: currentLink?.openInNewWindow ?? false,
    ...
    orderedList: listType === 'ordered_list',
    quote: hasAncestor(state, 'blockquote'),
    strikethrough: hasActiveMark(state, schema, 'strike'),
    underline: hasActiveMark(state, schema, 'underline'),
  };
}
