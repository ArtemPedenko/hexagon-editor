...
describe('basic Markdown extensions', () => {
  it('updates mounted content through transactions without recreating plugins', () => {
    ...
    const editor = mountBasicWysiwygEditor({
  ...
  it('round-trips definition lists, folding headings, and quote links', () => {
    ...
    expect(document.child(2).attrs).toMatchObject({
  ...
  it('changes a list item nesting level', () => {
    ...
    const state = EditorState.create({
      doc: document,
      selection: TextSelection.create(document, secondItemPosition),
    });
    ...
    expect(
      commands.liftListItem(state, (transaction) => {
        liftedState = state.apply(transaction);
      }),
    ).toBe(true);
    expect(basicMarkdownCodec.serialize(liftedState.doc)).toBe('* Первый пункт\n* Второй пункт');
    expect(
      commands.sinkListItem(liftedState, (transaction) => {
        nestedState = liftedState.apply(transaction);
      }),
    ).toBe(true);
  ...
  it('toggles bold for the selected text', () => {
    ...
    const document = basicMarkdownSchema.node('doc', null, [basicMarkdownSchema.node('paragraph', null, text)]);
    const state = EditorState.create({
      doc: document,
