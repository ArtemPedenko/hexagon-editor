...
export class MarkdownEditor implements MarkdownEditorInstance {
    ...
    get actions(): Readonly<EditorActions> { return this.#actions; }
    get currentMode(): MarkdownEditorMode { return this.#mode; }
    get preset(): MarkdownEditorPreset { return this.#preset; }
    get readonly(): boolean { return this.#readonly; }
    get toolbarVisible(): boolean { return this.#toolbarVisible; }

    append(markup: string): void { this.setValue(joinMarkdown(this.#value, markup)); }
    action(name: string): EditorAction<unknown, unknown> | undefined { return this.#actions[name]; }
    ...
    focus(): void { this.assertActive(); this.#focusHandler(); }
    getMode(): MarkdownEditorMode { return this.#mode; }
    getValue(): string { return this.#value; }
    hasFocus(): boolean { this.assertActive(); return this.#hasFocusHandler(); }
    insert(markup: string): void { this.append(markup); }
    isEmpty(): boolean { return this.#value.length === 0; }
    moveCursor(position: MarkdownEditorCursorPosition): void { this.assertActive(); this.#moveCursorHandler(position); }

    off<Key extends keyof MarkdownEditorEventMap>(type: Key, listener: (value: MarkdownEditorEventMap[Key]) => void): void {
        this.#events.off(type, listener);
    }

    on<Key extends keyof MarkdownEditorEventMap>(type: Key, listener: (value: MarkdownEditorEventMap[Key]) => void): void {
        this.assertActive();
        this.#events.on(type, listener);
    }

    prepend(markup: string): void { this.setValue(joinMarkdown(markup, this.#value)); }
    replace(markup: string): void { this.setValue(markup); }
    ...
    setMode(mode: MarkdownEditorMode): void { this.setEditorMode(mode); }
