...
export interface MarkdownParserDynamicModifierConfig {
    [elementType: string]: {processNode?: ProcessNode[]; processNodeAttrs?: ProcessNodeAttrs[]; processToken?: ProcessToken[]};
}
...
export class MarkdownParserDynamicModifier {
    ...
    processAttrs(token: Token, attrs: TokenAttrs): TokenAttrs {
        return this.#processors.get(tokenName(token.type))?.processNodeAttrs?.reduce(
            (current, process) => process(token, current), attrs,
        ) ?? attrs;
    }

    processNode(node: Node): Node {
        return this.#processors.get(node.type.name)?.processNode?.reduce(
            (current, process) => process(current), node,
        ) ?? node;
    }

    processTokens(tokens: Token[], rawMarkup: string | null): Token[] {
        return tokens.map((token, index) => {
            const processed = this.#processors.get(tokenName(token.type))?.processToken?.reduce(
                (current, process) => process(current, index, rawMarkup), token,
            ) ?? token;
...
export class MarkdownParser {
    ...
    constructor(schema: Schema, tokenizer: MarkdownIt, tokens: Record<string, ParserToken>, options: {
    ...
    parse(source: string | Token[]): Node {
        ...
        const tokens = this.#dynamicModifier?.processTokens(
        ...
        return this.#transforms.reduce((current, transform) => transform(current), document ?? this.#schema.topNodeType.createAndFill()!);
    }
    ...
    private addText(text: string): void {
        ...
        if (last?.isText && Mark.sameSet(last.marks, node.marks)) nodes[nodes.length - 1] = this.#schema.text(`${last.text ?? ''}${text}`, last.marks);
    ...
    private parseTokens(tokens: Token[]): void {
        for (const [index, token] of tokens.entries()) {
            if (token.type === 'text') { this.addText(trimToken(token.content)); continue; }
            if (token.type === 'inline') { this.parseTokens(token.children ?? []); continue; }
            if (token.type === 'softbreak' && this.#tokens.softbreak === undefined) { this.addText('\n'); continue; }
            ...
            const initialAttrs = spec.getAttrs?.(token, tokens, index) ?? (typeof spec.attrs === 'function' ? spec.attrs(token) : spec.attrs);
    ...
    private handleBlock(token: Token, type: NodeType, attrs: TokenAttrs | undefined, noCloseToken: boolean | undefined): void {
        if (noCloseToken) { this.#stack.push({attrs, content: [], type}); this.addText(trimToken(token.content)); this.closeNode(); return; }
        if (token.type.endsWith('_open')) this.#stack.push({attrs, content: [], type});
        if (token.type.endsWith('_close')) this.closeNode();
    }

    private handleMark(token: Token, type: MarkType, attrs: TokenAttrs | undefined, noCloseToken: boolean | undefined): void {
        const mark = type.create(attrs);
        if (noCloseToken) { this.#marks = mark.addToSet(this.#marks); this.addText(trimToken(token.content)); this.#marks = type.removeFromSet(this.#marks); return; }
...
function tokenName(name: string): string { return name.replace(/(_open|_close)$/, ''); }
function trimToken(value: string): string { return value.endsWith('\n') || value.endsWith('\\n') ? value.slice(0, -1) : value; }
