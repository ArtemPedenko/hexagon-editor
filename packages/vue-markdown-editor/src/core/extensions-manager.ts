...
export class ExtensionsManager {
  ...
  private createSchema(
    ...
    if (this.#options.schemaModifier !== undefined) {
      base.spec.nodes.forEach((name, spec) => {
        nodeSpecs = nodeSpecs.update(name, this.#options.schemaModifier?.processNodeSpec(name, spec) ?? spec);
      });
    }
  ...
  private createSerializer(
    ...
    for (const [name, entry] of nodes) {
      if (entry.toMd !== undefined)
        registry.addNode(name, entry.toMd as ConstructorParameters<typeof MarkdownSerializer>[0][string]);
    }
    for (const [name, entry] of marks) {
      if (entry.toMd !== undefined)
        registry.addMark(name, entry.toMd as ConstructorParameters<typeof MarkdownSerializer>[1][string]);
    }
    for (const [name, token] of extensionNodes)
      registry.addNode(name, token as ConstructorParameters<typeof MarkdownSerializer>[0][string]);
    for (const [name, token] of extensionMarks)
      registry.addMark(name, token as ConstructorParameters<typeof MarkdownSerializer>[1][string]);
