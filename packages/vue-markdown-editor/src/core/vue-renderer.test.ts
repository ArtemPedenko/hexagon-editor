...
describe('Vue ProseMirror renderer', () => {
  ...
  it('passes the parent app context to node views', () => {
    ...
    const view = new EditorView(target, {
      nodeViews: {
        image: createVueNodeView(component, { appContext: parentApp._context }),
      },
      state: EditorState.create({
        doc: basicMarkdownCodec.parse('![Image](https://example.com/image.png)'),
      }),
    });
  ...
  it('mounts and destroys a Vue widget decoration', () => {
    ...
    const plugin = new Plugin({
      props: {
        decorations: () => DecorationSet.create(documentNode, [decoration]),
      },
    });
    const view = new EditorView(target, {
  ...
  it('passes the parent app context to widget decorations', () => {
    ...
    const decoration = createVueWidgetDecoration(
      1,
      {
  ...
  it('shows a Vue context panel for a text selection', async () => {
    ...
    const panel = defineComponent({
      ...
      setup: (props) => () =>
        h('div', { class: 'vue-context-panel', 'data-visible': String(props.visible) }, props.selectedText),
    });
    const view = new EditorView(target, {
      state: EditorState.create({
        doc: documentNode,
  ...
  it('passes the parent app context to context panels', () => {
    ...
    const view = new EditorView(target, {
      state: EditorState.create({
        doc: documentNode,
        plugins: [
          createVueContextPanelPlugin(panel, {
            appContext: parentApp._context,
          }),
        ],
      }),
