...
export function createBasicDefaultPresetPlugins(
  schema: Schema,
  placeholder: string,
  selectionContext: SelectionContextOptions | undefined,
  onCancel?: () => boolean,
  onSubmit?: () => boolean,
): Plugin[] {
  return ExtensionsManager.plugins(
    (builder) =>
      builder.use(FullPreset, {
        bold: { boldKey: 'Mod-b' },
        editorModeKeymap: {
          ignoreKeysList: ['Tab', 'Shift-Tab'],
          onCancel,
          onSubmit,
        },
    ...
    props: {
      handleDOMEvents: {
        paste: (view, event) => {
          ...
          const table =
            document.childCount === 1 && document.firstChild?.type.name === TableNode.Table
              ? document.firstChild
              : undefined;
