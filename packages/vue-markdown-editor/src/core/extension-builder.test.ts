...
describe('ExtensionBuilder', () => {
    it('collects nodes, Markdown configuration, actions and prioritized plugins', () => {
        ...
        expect(spec.parserTokens().get('example_token')).toEqual({node: 'example'});
    ...
    it('rejects duplicate extension entities and actions', () => {
        const builder = new ExtensionBuilder().addNode('example', () => ({spec: {group: 'block'}}));
        ...
        expect(() => builder.addAction('example', () => ({
            isActive: () => false,
            isEnabled: () => true,
            metadata: () => undefined,
            run: () => undefined,
    ...
    it('overrides granular parser and serializer registrations', () => {
        const builder = new ExtensionBuilder()
            ...
            .overrideMarkdownTokenParserSpec('example_token', () => ({node: 'updated'}))
            .overrideNodeSerializerSpec('example', () => 'updated');
        const spec = builder.build();

        expect(spec.nodes().get('example')?.spec.atom).toBe(true);
        expect(spec.parserTokens().get('example_token')).toEqual({node: 'updated'});
