...
const unsubscribe = subscribeConfigure((config) => {
  globalLang.value = config.lang;
});
onBeforeUnmount(unsubscribe);
console.log(VERSION);
</script>
<template>
  <p>Global: {{ globalLang }}</p>
  <MarkdownEditor
    v-model="value"
    locale="en"
  />
...
editor({ readonly: true, mode: 'split', density: 2, hidden: false, missing: null, later: undefined }, [
  'surface',
  ['wide', null],
]);
